ar_fn_detectedFlareResponse = {
	...
	*/
	...
	{
		private ["_randomTarget","_reactionGroupsAlwaysArray"];
		_randomTarget = selectRandom _triggerList;
		[_randomTarget, "mil_circle", "ColorBLUFOR", "SPOTTED HERE", 0.7] call _fnc_debugMarkers;
		if ( sunOrMoon < 1) then 
		{
			_mortarArray = [_randomTarget] call ar_fn_closestMortarOpfor;
			if (count _mortarArray > 0) then 
			{
				[_mortarArray #0, _randomTarget, 250, 10, 15, 3, 200] spawn fnc_inaccurateArtyFlare;
			};
		};
		_reactionGroupsArray = missionNameSpace getVariable ["reactionGroupsOPFORArray", []]; // for now only OPFOR
		// cleanup, just to make sure no "grpNull" are in the array
		_reactionGroupsArray = _reactionGroupsArray - [grpNull];
		missionNameSpace setVariable ["reactionGroupsOPFORArray", _reactionGroupsArray];
		_reactionGroupsAlwaysArray = missionNameSpace getVariable ["reactionGroupsAlwaysOPFORArray", []]; // for now only OPFOR
		_reactionGroupsAlwaysArray = _reactionGroupsAlwaysArray - [grpNull];
		missionNameSpace setVariable ["reactionGroupsOPFORArray", _reactionGroupsAlwaysArray];
		// end of cleanup
		if ( _reactionGroups && (count _reactionGroupsArray > 0) ) then 
		{
			...
			_selectedGroupsArray = [_nearestGroup] + _reactionGroupsAlwaysArray;

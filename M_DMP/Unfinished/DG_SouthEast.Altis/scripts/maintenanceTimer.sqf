...
_executeTime = 30; // 15 seconds

_replaceFirstAidWithAce = {
	//params [_locationArray];

	{
		[_x] spawn araace_fnc_replaceFirstAidWithAceUnit;
	} forEach allUnits;

	{
		[_x] spawn araace_fnc_replaceFirstAidWithAceCargo;
	} forEach vehicles;
...
while {true} do // loops for entire duration that mission/server is running.
{
	...
	{
		[0,0,0] call _replaceFirstAidWithAce; // call the function.

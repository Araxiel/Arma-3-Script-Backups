...
_currentSector setVariable ["name", "Arizona"];
...
_currentSector setVariable ["previousRealOwnerSide",_currentSector getVariable "owner"];

_currentSector = sectorB;
_currentSector setVariable ["sides", [east]];
_currentSector setVariable ["owner", east];
_currentSector setVariable ["name", "Boston"];
_currentSector setVariable ["bis_fnc_addscriptedeventhandler_ownerchanged", [{[_this #0,_this #1, _this #2] spawn fnc_SOsectorCaptured}]];
